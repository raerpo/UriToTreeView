...
class App extends Component {
    ...
    constructor() {
        ...
        this.state = {
            contentURI: this.initialMockValue,
            contentJSON: this.URIToJSON(this.initialMockValue)
        };
    }

    URIToJSON = input => {
        const decodedValue = decodeURIComponent(input.replace(/\+/g, " "));
        try {
    ...
    handleInputJSON = evt => {
        const JSONValue = evt.target.value;
        this.setState({
            contentURI: this.JSONToURI(JSONValue),
            contentJSON: JSONValue
        });
    }

    handleInputURI = evt => {
        const URIValue = evt.target.value;
        this.setState({
            contentURI: URIValue,
            contentJSON: this.URIToJSON(URIValue) 
        });
    }
    ...
    render() {
        return (
            <div className="uriToTree">
                <Input
                  onChangeURI={this.handleInputURI}
                  contentURI={this.state.contentURI}
                  onChangeJSON={this.handleInputJSON}
                  contentJSON={this.state.contentJSON}
                />
                <Output json={this.state.contentJSON} />
            </div>
        );
    }
}
